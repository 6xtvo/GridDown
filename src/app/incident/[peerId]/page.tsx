...
export default function IncidentChatPage() {
	...
	return (
		<main className="flex min-h-screen flex-col items-center bg-black p-8 text-white">
			<div className="flex w-full max-w-4xl flex-col gap-4 border-2 border-red-600 bg-zinc-950 p-4 shadow-[0_0_15px_rgba(220,38,38,0.3)]">
				{/* Header */}
				<div className="flex items-center justify-between border-red-600 border-b pb-4">
					<button
						className="flex items-center gap-2 font-seven text-zinc-500 transition hover:text-red-500"
						onClick={() => router.push("/")}
					>
						◄ RETURN TO GRID
					</button>
					<div className="text-right font-seven">
						<div className="text-red-500 text-xl">
							ENCRYPTED COMMS: {targetPeerId}
						</div>
						<div
							className={`text-sm ${targetPeer ? "text-green-500" : "animate-pulse text-red-600"}`}
						>
							{targetPeer
								? "TARGET LOCK: ACQUIRED"
								: "TARGET OFFLINE / AWAITING PING..."}
						</div>
					</div>
				</div>
				...
				<div className="relative flex h-96 flex-col gap-3 overflow-y-auto border border-zinc-800 bg-black p-4 font-jetbrains">
					...
					{messages.map((msg, idx) => {
						...
						return (
							<div
								className={`flex max-w-[80%] flex-col ${isMe ? "items-end self-end" : "items-start self-start"}`}
								key={idx}
							>
								...
								<div
									className={`mt-1 border p-3 ${isMe ? "border-zinc-700 bg-zinc-900 text-zinc-300" : "border-red-900 bg-red-950/50 text-red-400"}`}
								>
									{msg.text}
								</div>
							</div>
				...
				<form className="flex gap-2" onSubmit={handleSend}>
					<input
						className="flex-1 border border-red-900 bg-black px-4 py-3 font-jetbrains text-red-500 focus:border-red-500 focus:outline-none"
					...
					/>
					<button
						className="border border-red-600 bg-red-900/20 px-8 font-seven text-red-500 text-xl transition hover:bg-red-600 hover:text-white disabled:opacity-50"
						disabled={!input.trim()}
						type="submit"
					>
						SEND
